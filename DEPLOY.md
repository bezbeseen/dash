...
6. **Inbound form webhook** (optional) — `INBOUND_FORM_WEBHOOK_SECRET` for **`POST /api/webhooks/inbound-form-lead`** (marketing automation). Optional **`INBOUND_CONVERSATION_WEBHOOK_SECRET`** for **`POST /api/webhooks/inbound-conversation`**; if omitted, the form secret is used for both. Disable Deployment Protection or allow those paths publicly.
