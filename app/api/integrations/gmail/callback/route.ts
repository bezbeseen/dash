...
<p>This usually means the tab sat open too long, you used a different browser, or you opened this URL manually. <strong>Start again from Dash:</strong> sidebar → <strong>Connect Gmail</strong>.</p>
...
<p><a href="/dashboard">← Back to dashboard</a></p>
  ...
  if (req.nextUrl.searchParams.get('error')) {
    return NextResponse.redirect(new URL('/dashboard?gmail_error=denied', req.nextUrl.origin));
  }

  const code = req.nextUrl.searchParams.get('code');
  if (!code) {
    return NextResponse.redirect(new URL('/dashboard?gmail_error=no_code', req.nextUrl.origin));
  }

  try {
    const { tokens } = await exchangeGmailCode(code, redirectUri);
    if (!tokens.refresh_token) {
      return NextResponse.redirect(new URL('/dashboard?gmail_error=no_refresh', req.nextUrl.origin));
    }
    ...
    try {
      ...
      if (status === 403 || reason === 'accessNotConfigured') {
        return NextResponse.redirect(new URL('/dashboard?gmail_error=gmail_api', req.nextUrl.origin));
      }
      return NextResponse.redirect(new URL('/dashboard?gmail_error=gmail_profile', req.nextUrl.origin));
    }

    if (!email) {
      return NextResponse.redirect(new URL('/dashboard?gmail_error=no_profile', req.nextUrl.origin));
    }
    ...
    if (!existingConn && mailboxCount >= 3) {
      return NextResponse.redirect(new URL('/dashboard?gmail_error=max_mailboxes', req.nextUrl.origin));
    }
    ...
    return NextResponse.redirect(new URL('/dashboard?gmail_connected=1', req.nextUrl.origin));
  } catch (e) {
    console.error('[gmail/callback] token exchange failed:', e);
    return NextResponse.redirect(new URL('/dashboard?gmail_error=token', req.nextUrl.origin));
  }
}
