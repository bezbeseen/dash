...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!job?.quickbooksEstimateId) {
    return NextResponse.json({ error: 'No QuickBooks estimate on this ticket.' }, { status: 404 });
  }
  ...
  try {
    const pdf = await fetchEstimatePdf(realm, job.quickbooksEstimateId);
    const safe = (job.quickbooksEstimateId || 'estimate').replaceAll(/[^\w.-]/g, '_');
    return new NextResponse(pdf, {
      status: 200,
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `inline; filename="estimate-${safe}.pdf"`,
