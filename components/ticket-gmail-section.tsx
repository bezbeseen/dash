import type { GmailConnection, GmailLinkSource } from '@prisma/client';
...
/** Find / unlink / paste / sync. Synced content is read in Correspondence, not here. */
  ...
  return (
    <section id={sectionId} className="ticket-detail-panel">
      ...
      <p className="meta ticket-doc-note">
        Thread plumbing: which mailbox, which Gmail conversation, find / unlink / sync. Read the
        mail in <strong>Correspondence</strong> above.
      </p>
      ...
      {syncedOk ? (
        <p className="board-toast board-toast-ok" style={{ marginBottom: 12 }}>
          Gmail thread synced. Messages are in Correspondence above.
        </p>
      ) : null}
      {matchError ? (
        <p className="board-toast board-toast-error" style={{ marginBottom: 12 }}>
      ...
      <p className="meta" style={{ marginBottom: 12 }}>
        {gmailThreadId ? (
          <>
            Thread linked
            {linkSource ? ` (${LINK_SOURCE_LABEL[linkSource]})` : ''}.{' '}
            {gmailMessageTotalCount === 0
              ? 'No messages stored yet — open Advanced and sync.'
              : `${gmailMessageTotalCount} synced message${gmailMessageTotalCount === 1 ? '' : 's'} in Correspondence.`}
          </>
        ) : (
          'No Gmail thread linked yet.'
        )}
      </p>

      <details className="gmail-advanced-details" open={advancedOpen || undefined}>
        <summary className="gmail-advanced-summary">Advanced — find, unlink, paste a thread ID, or sync</summary>
        <div className="gmail-advanced-body">
          ...
          <p className="meta ticket-doc-note" style={{ marginTop: -6 }}>
            ...
            <strong>does not</strong> download mail.
          </p>
          <p className="meta ticket-doc-note" style={{ marginTop: -6 }}>
            <strong>Demo / sandbox QuickBooks tickets are fine:</strong> Gmail still talks to your{' '}
            <strong>real Google mailbox</strong>. You’re only choosing which email thread to attach to this ticket row
            in Dash — the QB customer on the card doesn’t need to match the email participants.
          </p>
          ...
          <div className="gmail-auto-match" style={{ marginBottom: 20 }}>
            ...
            {gmailThreadId && linkSource === 'YELP_EMAIL' ? (
              <p className="meta" style={{ marginTop: 8, marginBottom: 0 }}>
                Yelp notification emails for this conversation show in <strong>Correspondence</strong> above. Replies
                that only exist inside Yelp Biz and were never emailed will not appear there.
              </p>
            ) : null}
          </div>
                ...
                {ranked.map((s) => (
                  <li key={s.threadId} className="gmail-message-card">
                    <div className="gmail-message-head">
                      ...
                      <span className="meta">{fmtDetailDate(s.lastMessageAt ? new Date(s.lastMessageAt) : null)}</span>
                    </div>
